...
function parseReactCode(code, isTypeScript) {
    try {
        const plugins = ['jsx', 'classProperties', 'objectRestSpread', 'dynamicImport', 'decorators-legacy', 'asyncGenerators', 'exportDefaultFrom', 'exportNamespaceFrom', 'optionalChaining', 'nullishCoalescingOperator',];
...
function generateCodeFromAst(ast) {
    try {
        const result = babelGenerator(ast, {
            retainLines: true, comments: true, compact: false, jsescOption: {
                quotes: 'single'
            }
        });
...
function fixHookDependencies(ast, options) {
    babelTraverse(ast, {
        CallExpression(path) {
            ...
            if (!babelTypes.isIdentifier(node.callee) || (node.callee.name !== 'useEffect' && node.callee.name !== 'useCallback' && node.callee.name !== 'useMemo')) {
                return;
            }
            ...
            babelTraverse(callback, {
                Identifier(idPath) {
                    ...
                    if (idPath.scope.hasBinding(idName) || ['document', 'window', 'console', 'Math', 'JSON', 'parseInt', 'undefined'].includes(idName)) {
                        return;
                    }
            ...
            node.arguments[1] = babelTypes.arrayExpression(dependencies.map(dep => babelTypes.identifier(dep)));
        }
    });
}
...
function extractComponent(ast, options) {
    ...
    function _containsJSXReturn(blockStatement) {
        for (const statement of blockStatement.body) {
            if (statement.type === 'ReturnStatement' && statement.argument && (statement.argument.type === 'JSXElement' || statement.argument.type === 'JSXFragment')) {
                return true;
            }
        }
        return false;
    ...
    function _extractNestedComponents(path) {
        ...
        path.traverse({
            JSXElement(jsxPath) {
                ...
                if (jsxPath.parent.type === 'ReturnStatement' && jsxPath.parentPath.parent === path.node.body) {
                    return;
                }

                // Don't extract small elements (less than 5 lines)
                if (!jsxPath.node.loc || (jsxPath.node.loc.end.line - jsxPath.node.loc.start.line < 5)) {
                    return;
                }

                // Check if it has multiple children or props
                const hasMultipleChildren = jsxPath.node.children && jsxPath.node.children.length > 2;
                const hasMultipleProps = jsxPath.node.openingElement.attributes && jsxPath.node.openingElement.attributes.length > 2;
        ...
        const elementsToExtract = jsxElements.slice(0, Math.min(3, jsxElements.length));
        ...
        for (let i = 0; i < elementsToExtract.length; i++) {
            ...
            if (elementType[0] === elementType[0].toUpperCase()) {
                ...
                extractedName = elementType[0].toUpperCase() + elementType.slice(1) + 'Section';
            }
            ...
            jsxPath.traverse({
                Identifier(idPath) {
                    ...
                    if (idPath.parent.type === 'JSXIdentifier' || (idPath.parent.type === 'JSXAttribute' && idPath.key === 'name')) {
                        return;
                    }
            ...
            for (const varName of usedVariables) {
                props.push(babelTypes.objectProperty(babelTypes.identifier(varName), babelTypes.identifier(varName), false, true // shorthand
                ));
            }

            // Create a new component function
            const newComponent = babelTypes.functionDeclaration(babelTypes.identifier(extractedName), [babelTypes.identifier('props')], babelTypes.blockStatement([babelTypes.returnStatement(jsxPath.node)]));

            // Replace the JSX with a call to the new component
            const propsObj = babelTypes.objectExpression(props);
            const newJSX = babelTypes.jsxElement(babelTypes.jsxOpeningElement(babelTypes.jsxIdentifier(extractedName), props.length > 0 ? [babelTypes.jsxAttribute(babelTypes.jsxIdentifier('props'), babelTypes.jsxExpressionContainer(propsObj))] : [], true), null, [], true);
                    ...
                    if (styleObj.properties.length > 3) {
                        ...
                        styleObj.properties.forEach((prop, index) => {
                            ...
                            if (prop.loc) {
                                ...
                                prop.loc.end.column = prop.loc.start.column + prop.key.name.length +
                                    prop.value.loc.end.column - prop.value.loc.start.column + 2;
                            }
                        });
                    }
                }
        ...
        JSXElement(path) {
            ...
            if (attrs.length > 3 && lineBreakRule === 'multiline') {
                ...
                if (path.node.closingElement && path.node.closingElement.loc) {
                    path.node.closingElement.loc.start.column = path.node.loc.start.column;
                }
            }
        },
                ...
                SwitchStatement(path) {
                    if (path.node.id === astNodeId) {
                        ...
                        if (defaultCase === null && ensureDefault) {
                            ...
                            defaultCase = babelTypes.switchCase(
                                null,
