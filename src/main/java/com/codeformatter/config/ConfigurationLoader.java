...
public class ConfigurationLoader {
  ...
  /**
   * Loads configuration from a file with fallback to defaults. Improved error handling and
   * validation.
   */
  public static FormatterConfig loadConfig(Path configPath) {
    ...
    if (!Files.exists(configPath)) {
      logger.warning(
          "Configuration file not found: " + configPath + ", using default configuration");
      return loadDefaultConfig();
    }
    ...
    try {
      ...
      logger.info(
          "Configuration loaded successfully with "
              + formatterConfig.getPluginConfigsMap().size()
    ...
    if (config.containsKey("plugins") && config.get("plugins") instanceof Map) {
      ...
      for (Map.Entry<String, Object> entry : pluginsMap.entrySet()) {
        if (entry.getValue() instanceof Map) {
          ...
          logger.warning(
              "Invalid configuration for plugin '" + entry.getKey() + "', using defaults");
  ...
  private static void _validateConfigurationValues(
      Map<String, Object> generalConfig, Map<String, Map<String, Object>> pluginConfigs) {
