...
/**
 * Represents a JavaScript/React Abstract Syntax Tree. This class acts as a bridge between Java and
 * the JavaScript AST produced by Babel.
 */
public class JsAst {
  ...
  /**
   * Enhanced utility class for traversing AST nodes and finding parent nodes This can be used by
   * analyzers when parent node information is needed
   */
  public static class AstNodeFinder {
    ...
    /** Gets the parent node of a given node Lazily builds the parent map on first use */
    ...
    /**
     * Traverses the AST once to build a map of parent-child relationships This is more efficient
     * than traversing for each lookup
     */
    private void buildParentMap() {
      ...
      try {
        ...
        Value jsParentMap =
            ast.getAstValue()
                .getMember("context")
                .invokeMember("eval", "buildParentMap(currentAst)");
