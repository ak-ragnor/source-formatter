...
public class HookUsageAnalyzer implements ReactCodeAnalyzer {
  ...
  private void _checkHookRules(
      JsAst ast, JsAst.AstNodeFinder nodeFinder, List<FormatterError> errors) {
      ...
      if (!_isInComponentOrHook(ast, nodeFinder, hookCall)) {
        errors.add(
            new FormatterError(
                ...
                "React Hook '"
                    + _getHookName(hookCall)
                    + "' is called in a regular function. "
        ...
        if (funcName != null && !funcName.startsWith("use")) {
          errors.add(
              new FormatterError(
                  ...
                  "Function '"
                      + funcName
                      + "' calls React hooks but doesn't start with 'use'. "
  ...
  private void _checkEffectDependencies(
      JsAst ast, Value[] effectCalls, List<FormatterError> errors) {
  ...
  private void _checkCallbackDependencies(
      JsAst ast, Value[] callbackCalls, List<FormatterError> errors) {
  ...
  private String _getFunctionName(JsAst ast, Value func) {
    if (func.hasMember("id")
        && !func.getMember("id").isNull()
        && func.getMember("id").hasMember("name")) {
    ...
    try {
      ...
      if (body.hasMember("body") && body.getMember("body").hasArrayElements()) {
        for (int i = 0; i < body.getMember("body").getArraySize(); i++) {
          ...
          if (stmt.hasMember("type")
              && stmt.getMember("type").asString().equals("VariableDeclaration")) {
            ...
            for (int j = 0; j < declarations.getArraySize(); j++) {
              ...
              if (decl.hasMember("id")
                  && decl.getMember("id").hasMember("name")
