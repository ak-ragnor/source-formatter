...
public class ComponentStructureAnalyzer implements ReactCodeAnalyzer {
      ...
      if (body.hasMember("body") && body.getMember("body").hasArrayElements()) {
        for (int i = 0; i < body.getMember("body").getArraySize(); i++) {
          ...
          if (stmt.hasMember("type")
              && stmt.getMember("type").asString().equals("ReturnStatement")) {
  ...
  private boolean _isClassComponent(JsAst ast, Value node) {
    ...
    if (superClass.hasMember("type")
        && superClass.getMember("type").asString().equals("Identifier")) {
    ...
    if (superClass.hasMember("type")
        && superClass.getMember("type").asString().equals("MemberExpression")) {
      Value object = superClass.getMember("object");
      Value property = superClass.getMember("property");

      if (object.hasMember("type")
          && object.getMember("type").asString().equals("Identifier")
          && property.hasMember("type")
          && property.getMember("type").asString().equals("Identifier")) {
  ...
  private void _analyzeComponentStructure(
      JsAst ast, Value component, String componentType, List<FormatterError> errors) {
    String componentName = "Unknown";
    if (component.hasMember("id") && !component.getMember("id").isNull()) {
      componentName = ast.getStringProperty(component.getMember("id"), "name");
    } else if (componentType.equals("function")
        && component.hasMember("parent")
        && !component.getMember("parent").isNull()) {
  ...
  private int _calculateJsxDepth(Value jsxElement) {
    if (!jsxElement.hasMember("children") || !jsxElement.getMember("children").hasArrayElements()) {
      return 1;
    }
    ...
    for (int i = 0; i < jsxElement.getMember("children").getArraySize(); i++) {
      ...
      if (child.hasMember("type") && child.getMember("type").asString().equals("JSXElement")) {
  ...
  private boolean _checkLifecycleComplexity(JsAst ast, Value classComponent) {
    if (!classComponent.hasMember("body") || !classComponent.getMember("body").hasMember("body")) {
      return false;
    }
    ...
    for (int i = 0; i < classBody.getArraySize(); i++) {
      ...
      if (node.hasMember("type") && node.getMember("type").asString().equals("ClassMethod")) {

        if (node.hasMember("key") && node.getMember("key").hasMember("name")) {
  ...
  private boolean _isMethodComplex(Value method) {
    if (!method.hasMember("body") || !method.getMember("body").hasMember("body")) {
      return false;
    }
