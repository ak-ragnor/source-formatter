...
/**
 * Simplified analyzer that detects design pattern violations. It only identifies potential issues
 * without implementing complex refactoring logic.
 */
public class DesignPatternAnalyzer implements CodeAnalyzer {
  ...
  private void _checkSingleResponsibilityPrinciple(
  ...
  private void _checkFactoryPattern(
      ClassOrInterfaceDeclaration clazz, List<FormatterError> errors) {

    List<MethodDeclaration> factoryMethodCandidates =
  ...
  private void _checkBuilderPattern(
      ClassOrInterfaceDeclaration clazz, List<FormatterError> errors) {
    List<String> fieldNames =
        clazz.getFields().stream()
            .flatMap(f -> f.getVariables().stream().map(NodeWithSimpleName::getNameAsString))
            .toList();

    if (fieldNames.size() >= 5) {
      boolean hasComplexConstructor =
          clazz.getConstructors().stream().anyMatch(c -> c.getParameters().size() >= 4);
